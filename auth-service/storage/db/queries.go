...
func (d DbInfo) InsertUser(mapping models.DBUser) (int, error) {
	...
	err := d.dbClient.QueryRowContext(ctx, query,
		mapping.FirstName,
		mapping.LastName,
		mapping.Email,
		mapping.Password,
...
func (d *DbInfo) GetUserByEmailid(email string) (models.DBUser, error) {
	ctx, cancel := context.WithTimeout(context.Background(), dbTimeout)
	defer cancel()
	...
	row := d.dbClient.QueryRowContext(ctx, query, email)
...
func (d *DbInfo) UpdateUser(email, newpasswd string) error {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()

	query := `update users set password = $2, updated_at = $3 where email = $1`

	_, err := d.dbClient.ExecContext(ctx, query,
		email,
		newpasswd,
		time.Now(),
	)
